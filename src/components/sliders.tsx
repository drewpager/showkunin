...
export function Volume() {
  return (
    <VolumeSlider.Root className="group relative inline-flex h-10 w-full max-w-[80px] cursor-pointer select-none items-center outline-none">
      ...
      <VolumeSlider.Thumb className="absolute left-[var(--slider-fill)] top-1/2 block h-[15px] w-[15px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#cacaca] bg-white opacity-0 outline-none ring-white/40 transition-opacity will-change-[left] focus:opacity-100 focus:ring-4 group-hover:opacity-100 data-[dragging]:opacity-100" />
    </VolumeSlider.Root>
  );
}
...
export function Time({ thumbnails }: TimeSliderProps) {
  return (
    <TimeSlider.Root className="group relative inline-flex h-9 w-full cursor-pointer select-none items-center outline-none">
